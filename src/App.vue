...
<template>
  <main>
    ...
    <form @submit.prevent="searchAnime">
      <input 
          type="text" 
          placeholder="¿Qué anime estás viendo?"
          v-model="query" 
          @input="handleInput"
      />
    ...
    <div class="results" v-if="search_results.length > 0">
      <div v-for="anime in search_results" class="result">
        ...
        <div class="details">
          ...
          <button @click="addAnime(anime)" class="button">Agregar a mi lista</button>
        </div>
      </div>
    </div>
...
* {
    ...
    font-family: 'Glowing Candy', 'Fjalla One';
}
...
.button {
  ...
  display: block;
  padding: 0.5rem 1rem;
  background-image: linear-gradient(to right, deeppink 50%, #B15EFF 50%);
  background-size: 200%;
  color: white;
  font-size: 1.125rem;
  font-weight: bold;
